...
const SIG_ALGO_OPTIONS: NetworkOption[] = [
  { key: "ECDSA_secp256k1", label: "ECDSA secp256k1" },
  { key: "ECDSA_P256", label: "ECDSA P256" },
];

const HASH_ALGO_OPTIONS: NetworkOption[] = [
...
export default function Settings() {
  ...
  const saveConfig = (overrides: Record<string, any> = {}) => {
  ...
  const handleNetworkChange = (network: string) => {
    ...
    setStore((s: Record<string, any>) => ({ ...s, network, rpcUrl: newRpc }));
  ...
  const handleRpcChange = (value: string) => {
    ...
    setStore((s: Record<string, any>) => ({ ...s, rpcUrl: value }));
    saveConfig({ rpcUrl: value });
  };

  const handleAutoSignChange = (value: boolean) => {
    setAutoSign(value);
    setStore((s: Record<string, any>) => ({ ...s, autoSign: value }));
    saveConfig({ autoSign: value });
  };

  const handleSigAlgoChange = (value: string) => {
    setSigAlgo(value);
    setStore((s: Record<string, any>) => ({ ...s, sigAlgo: value }));
    saveConfig({ sigAlgo: value });
  };

  const handleHashAlgoChange = (value: string) => {
    setHashAlgo(value);
    setStore((s: Record<string, any>) => ({ ...s, hashAlgo: value }));
