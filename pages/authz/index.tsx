...
const Authz = () => {
  ...
  const [authzInfo, setAuthzInfo] = useState<any>(null);
  const autoSigned = useRef(false);

  useEffect(() => {
    const callback = (msg: any) => {
      console.log("[authz] msg ==>", msg);
      setAuthzInfo(msg);
    };
    try {
  ...
  const onApproval = async () => {
    const result = await signWithKey(store, authzInfo.body.message);
    const response: any = {
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>Flow Dev Wallet - Transaction</title>
      </Head>
            ...
            {authzInfo && (
              <CardContent className="flex flex-col space-y-4 p-6">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-gray-400">Cadence</label>
                  <Textarea
                    readOnly
